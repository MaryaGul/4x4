...
let RolesGuard = class RolesGuard {
    ...
    canActivate(context) {
        try {
            const requiaredRoles = this.reflector.getAllAndOverride(roles_auth_decorator_1.ROLES_KEY, [
                context.getHandler(),
                context.getClass(),
            ]);
            if (!requiaredRoles) {
            ...
            if (bearer !== 'Bearer' || !token) {
                throw new common_1.UnauthorizedException({ message: "User not authorizated" });
            }
